...
export function registerLog(activity: Command): void {
  activity.addCommand(
    defineCommand({
      use: "log",
      short:
        "Show recent activity log for workspace or a page (v3 desktop session required)",
      options: {
        page: { type: "string", description: "Scope to a specific page" },
        limit: { type: "string", default: "20", description: "Number of activity entries" },
      },
          ...
          const entries = (result.activityIds ?? []).map((actId) => {
            ...
            return {
              ...
              startTime: activity.start_time
                ? new Date(activity.start_time).toISOString()
                : undefined,
              endTime: activity.end_time
                ? new Date(activity.end_time).toISOString()
                : undefined,
            };
          });
